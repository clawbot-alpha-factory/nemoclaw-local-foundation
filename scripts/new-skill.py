...
def gen_run(args, names, llm, critic):
    ...
    L = ['#!/usr/bin/env python3', '"""',
         ...
         'from datetime import datetime, timezone', '', '',
         '# L-003 compliant: route all LLM calls through lib/routing (config-driven)',
         'from lib.routing import call_llm as _routed_call, resolve_from_env_or_config', '', '',
         'def call_resolved(messages, context, max_tokens=4000):',
         '    """Route LLM call through config/routing/routing-config.yaml."""',
    ...
    for i in range(1, n + 1):
        ...
        if is_last:
            ...
            L += [f'def step_{i}_critic(inputs, context):',
                  ...
                  f'    if error: content, error = _routed_call(messages, task_class="structured_short", max_tokens=1000)',
            ...
            L += [f'    role = context.get("execution_role", "")',
                  ...
                  f'    if error: content, error = _routed_call(messages, task_class="general_short", max_tokens=4000)',
                  f'    if error: return None, error',
