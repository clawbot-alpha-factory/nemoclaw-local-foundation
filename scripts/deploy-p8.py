#!/usr/bin/env python3
"""
NemoClaw P-8 Deployment: Skills Marketplace

Creates: skill_marketplace_service.py, marketplace.py (router)
Patches: main.py (import + init + router registration)

Run from repo root:
    cd ~/nemoclaw-local-foundation
    python3 scripts/deploy-p8.py
"""
...
# ═══════════════════════════════════════════════════════════════════
# FILE 1: skill_marketplace_service.py
# ═══════════════════════════════════════════════════════════════════
...
def deploy():
    ...
    # 1. Write service
    print("1/3 Writing skill_marketplace_service.py...")
    svc_path = BACKEND / "app" / "services" / "skill_marketplace_service.py"
    svc_path.write_text(SERVICE_CODE.strip() + "\n")
    try:
    ...
    # 2. Write router
    print("2/3 Writing marketplace.py router...")
    router_path = BACKEND / "app" / "api" / "routers" / "marketplace.py"
    ...
    # Import
    import_old = "from app.api.routers import skill_factory as skill_factory_router"
    import_new = """from app.api.routers import skill_factory as skill_factory_router

# ── P-8: Skills Marketplace ──
from app.api.routers import marketplace as marketplace_router"""

    if "marketplace_router" not in content:
        if import_old in content:
            content = content.replace(import_old, import_new)
            patched += 1
        else:
            errors.append("Import target not found")
    else:
        patched += 1

    # Init
    init_old = '    logger.info("E-5: SkillFactoryService initialized")\n\n    # ── E-7b:'
    init_new = '''    logger.info("E-5: SkillFactoryService initialized")
    ...
    if "SkillMarketplaceService" not in content:
        if init_old in content:
            content = content.replace(init_old, init_new)
            patched += 1
        else:
            errors.append("Init target not found")
    else:
        patched += 1

    # Router registration
    reg_old = "app.include_router(skill_factory_router.router)  # E-5: Skill Factory"
    reg_new = """app.include_router(skill_factory_router.router)  # E-5: Skill Factory
app.include_router(marketplace_router.router)  # P-8: Skills Marketplace"""

    if "marketplace_router.router" not in content:
    ...
    if errors:
        ...
        print("✅ P-8 deployed successfully")
        print()
        ...
        print('    http://127.0.0.1:8100/api/marketplace/sources | python3 -m json.tool')
        print()
        print('  # Add source')
        print('  curl -s -X POST -H "Authorization: Bearer $TOKEN" \\')
        print('    -H "Content-Type: application/json" \\')
        print("    -d '{\"url\":\"https://github.com/clawbot-alpha-factory/nemoclaw-local-foundation\",\"name\":\"self\"}' \\")
        print('    http://127.0.0.1:8100/api/marketplace/sources | python3 -m json.tool')
        print()
        print('  # Discover')
        print('  curl -s -H "Authorization: Bearer $TOKEN" \\')
        print('    "http://127.0.0.1:8100/api/marketplace/discover?force=true" | python3 -c "import json,sys; d=json.load(sys.stdin); print(f\\"Found {d[\'total\']} skills\\")"')
        print()
        print('  # Stats')
        print('  curl -s -H "Authorization: Bearer $TOKEN" \\')
        print('    http://127.0.0.1:8100/api/marketplace/stats | python3 -m json.tool')
        print()
        print('  # Check updates')
        print('  curl -s -H "Authorization: Bearer $TOKEN" \\')
        print('    http://127.0.0.1:8100/api/marketplace/updates | python3 -m json.tool')
        print()
        print('  cd ~/nemoclaw-local-foundation && bash scripts/full_regression.sh')
        print()
        print('  git add -A && git status')
        print('  git commit -m "feat(engine): P-8 skills marketplace — discover, install, update, uninstall from GitHub repos"')
