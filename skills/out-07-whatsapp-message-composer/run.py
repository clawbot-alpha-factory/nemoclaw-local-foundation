#!/usr/bin/env python3
"""
NemoClaw Skill — WhatsApp Message Composer v1.0.0 (out-07-whatsapp-message-composer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-07-whatsapp-message-composer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("recipient_context") or len(spec["inputs"]["recipient_context"]) < 10:
        errors.append("Missing required input: recipient_context")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    recipient_context = spec["inputs"].get("recipient_context", "")
    language = spec["inputs"].get("language", "en")
    context = f"""Task: WhatsApp Message Composer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a WhatsApp business communication expert specializing in MENA markets. Write messages that build trust and relationships before selling."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-07-whatsapp-message-composer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-07-whatsapp-message-composer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-07-whatsapp-message-composer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-07-whatsapp-message-composer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-07-whatsapp-message-composer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: WhatsApp Message Composer v1.0.0\nThread: {wf_id}")
