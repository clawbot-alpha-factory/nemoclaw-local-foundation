#!/usr/bin/env python3
"""
NemoClaw Skill — Email Campaign Designer v1.0.0 (k44-email-campaign-designer)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k44-email-campaign-designer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("campaign_brief") or len(spec["inputs"]["campaign_brief"]) < 20:
        errors.append("Missing required input: campaign_brief")
    if errors:
...
def step_1_parse(state):
    campaign_brief = spec["inputs"].get("campaign_brief", "")
    num_emails = spec["inputs"].get("num_emails", "5")
    
    context = f"""Task: Email Campaign Designer
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are an email marketing specialist. Design high-converting email sequences with compelling copy."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k44-email-campaign-designer_{wf_id}_{ts}.md"
    ...
    env_name = f"k44-email-campaign-designer_{wf_id}_{ts}_envelope.json"
    ...
    envelope = {
        "skill_id": "k44-email-campaign-designer",
        "workflow_id": wf_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k44-email-campaign-designer")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k44-email-campaign-designer_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Email Campaign Designer v1.0.0")
    ...
    if final and final.values:
        ...
        if vals.get("artifact_path"):
            print(f"\nSkill complete: Email Campaign Designer")
            print(f"Thread ID: {wf_id}")
