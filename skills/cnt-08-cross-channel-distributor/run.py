#!/usr/bin/env python3
"""
NemoClaw Skill — Cross-Channel Distributor v1.0.0 (cnt-08-cross-channel-distributor)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-08-cross-channel-distributor-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("content_piece") or len(spec["inputs"]["content_piece"]) < 20:
        errors.append("Missing required input: content_piece")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    content_piece = spec["inputs"].get("content_piece", "")
    channels = spec["inputs"].get("channels", "linkedin,instagram,twitter,email")
    context = f"""Task: Cross-Channel Distributor
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a content distribution specialist. Optimize content for each platform and coordinate cross-channel publishing for maximum reach."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-08-cross-channel-distributor_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-08-cross-channel-distributor_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-08-cross-channel-distributor", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-08-cross-channel-distributor"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-08-cross-channel-distributor_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Cross-Channel Distributor v1.0.0\nThread: {wf_id}")
