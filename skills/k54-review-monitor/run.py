#!/usr/bin/env python3
"""
NemoClaw Skill — Review Monitor v1.0.0 (k54-review-monitor)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k54-review-monitor-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("review_data") or len(spec["inputs"]["review_data"]) < 20:
        errors.append("Missing required input: review_data")
    if errors:
...
def step_1_parse(state):
    review_data = spec["inputs"].get("review_data", "")
    product_name = spec["inputs"].get("product_name", "NemoClaw")
    
    context = f"""Task: Review Monitor
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a reputation and review analyst. Synthesize review data into actionable insights."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k54-review-monitor_{wf_id}_{ts}.md"
    ...
    env_name = f"k54-review-monitor_{wf_id}_{ts}_envelope.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k54-review-monitor")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k54-review-monitor_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Review Monitor v1.0.0")
