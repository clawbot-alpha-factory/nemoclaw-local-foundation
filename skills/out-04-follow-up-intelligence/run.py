#!/usr/bin/env python3
"""
NemoClaw Skill — Follow-Up Intelligence v1.0.0 (out-04-follow-up-intelligence)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-04-follow-up-intelligence-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("conversation_history") or len(spec["inputs"]["conversation_history"]) < 20:
        errors.append("Missing required input: conversation_history")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    conversation_history = spec["inputs"].get("conversation_history", "")
    deal_stage = spec["inputs"].get("deal_stage", "outreach")
    context = f"""Task: Follow-Up Intelligence
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a conversation intelligence analyst. Read between the lines of prospect replies and recommend the optimal next move."""), ("human", f"""{context}{fb}
...
def step_3_critic(state):
    ...
    if score < 7.0:
        try:
            from langchain_anthropic import ChatAnthropic
            resp = ChatAnthropic(model="claude-haiku-4-5-20251001", max_tokens=300).invoke([("system","Score 1-10 as JSON: {\"score\":N,\"feedback\":\"...\"}"),("human",f"Task: Follow-Up Intelligence\n\nOutput:\n{output[:2000]}")])
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-04-follow-up-intelligence_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-04-follow-up-intelligence_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-04-follow-up-intelligence", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-04-follow-up-intelligence"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-04-follow-up-intelligence_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Follow-Up Intelligence v1.0.0\nThread: {wf_id}")
