#!/usr/bin/env python3
"""
NemoClaw Skill — Social Poster v1.0.0 (k53-social-poster)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k53-social-poster-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a social media content strategist. Create engaging, platform-optimized social posts."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k53-social-poster_{wf_id}_{ts}.md"
    ...
    env_name = f"k53-social-poster_{wf_id}_{ts}_envelope.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k53-social-poster")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k53-social-poster_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Social Poster v1.0.0")
