#!/usr/bin/env python3
"""
NemoClaw Skill — Revenue Loop Enforcer v1.0.0 (rev-14-revenue-loop-enforcer)
...
def call_llm(messages, max_tokens=4000):
    ...
    return call_llm_or_chain(messages, task_class="moderate", task_domain="strategic_reasoning", max_tokens=max_tokens)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-14-revenue-loop-enforcer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("active_opportunities") or len(spec["inputs"]["active_opportunities"]) < 20:
        errors.append("Missing required input: active_opportunities")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    active_opportunities = spec["inputs"].get("active_opportunities", "")
    loop_stages = spec["inputs"].get("loop_stages", "demand,offer,content,outreach,close,payment,feedback")
    context = f"""Task: Revenue Loop Enforcer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a revenue operations enforcer. Audit every active opportunity and ensure no step in the revenue cycle is skipped or stalled."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-14-revenue-loop-enforcer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-14-revenue-loop-enforcer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-14-revenue-loop-enforcer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-14-revenue-loop-enforcer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-14-revenue-loop-enforcer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Revenue Loop Enforcer v1.0.0\nThread: {wf_id}")
