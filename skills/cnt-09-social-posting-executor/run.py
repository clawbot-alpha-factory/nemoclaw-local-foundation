#!/usr/bin/env python3
"""
NemoClaw Skill — Social Posting Executor v1.0.0 (cnt-09-social-posting-executor)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-09-social-posting-executor-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("platform") or len(spec["inputs"]["platform"]) < 3:
        errors.append("Missing required input: platform")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    post_content = spec["inputs"].get("post_content", "")
    platform = spec["inputs"].get("platform", "")
    context = f"""Task: Social Posting Executor
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a social media posting specialist. Format content for each platform's requirements and execute posting with performance tracking."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-09-social-posting-executor_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-09-social-posting-executor_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-09-social-posting-executor", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-09-social-posting-executor"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-09-social-posting-executor_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Social Posting Executor v1.0.0\nThread: {wf_id}")
