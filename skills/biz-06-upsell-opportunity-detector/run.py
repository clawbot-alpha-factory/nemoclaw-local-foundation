#!/usr/bin/env python3
"""
NemoClaw Skill — Upsell Opportunity Detector v1.0.0 (biz-06-upsell-opportunity-detector)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-biz-06-upsell-opportunity-detector-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("client_usage_data") or len(spec["inputs"]["client_usage_data"]) < 20:
        errors.append("Missing required input: client_usage_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    client_usage_data = spec["inputs"].get("client_usage_data", "")
    service_catalog = spec["inputs"].get("service_catalog", "")
    context = f"""Task: Upsell Opportunity Detector
- client_usage_data: {client_usage_data}
...
def step_2_generate(state):
    ...
    messages = [("system", """You are an expansion revenue specialist. Identify upsell opportunities based on client behavior and unmet needs."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"biz-06-upsell-opportunity-detector_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"biz-06-upsell-opportunity-detector_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "biz-06-upsell-opportunity-detector", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="biz-06-upsell-opportunity-detector"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"biz-06-upsell-opportunity-detector_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Upsell Opportunity Detector v1.0.0\nThread: {wf_id}")
