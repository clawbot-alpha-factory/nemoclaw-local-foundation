#!/usr/bin/env python3
"""
NemoClaw Skill — System Learning Engine v1.0.0 (rev-19-system-learning-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-19-system-learning-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("system_performance_data") or len(spec["inputs"]["system_performance_data"]) < 20:
        errors.append("Missing required input: system_performance_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    system_performance_data = spec["inputs"].get("system_performance_data", "")
    learning_period = spec["inputs"].get("learning_period", "last_7_days")
    context = f"""Task: System Learning Engine
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a machine learning operations analyst. Identify causal patterns between system actions and revenue outcomes. Recommend priority changes based on what actually works."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-19-system-learning-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-19-system-learning-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-19-system-learning-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-19-system-learning-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-19-system-learning-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: System Learning Engine v1.0.0\nThread: {wf_id}")
