#!/usr/bin/env python3
"""
NemoClaw Skill — Growth Experiment Designer v1.0.0 (scl-06-growth-experiment-designer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-scl-06-growth-experiment-designer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("growth_goal") or len(spec["inputs"]["growth_goal"]) < 10:
        errors.append("Missing required input: growth_goal")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    growth_goal = spec["inputs"].get("growth_goal", "")
    constraints = spec["inputs"].get("constraints", "budget_limited")
    context = f"""Task: Growth Experiment Designer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a growth hacking specialist. Design rapid experiments with clear hypotheses, metrics, and timelines that drive measurable growth."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"scl-06-growth-experiment-designer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"scl-06-growth-experiment-designer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "scl-06-growth-experiment-designer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="scl-06-growth-experiment-designer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"scl-06-growth-experiment-designer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Growth Experiment Designer v1.0.0\nThread: {wf_id}")
