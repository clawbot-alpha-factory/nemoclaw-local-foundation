#!/usr/bin/env python3
"""
NemoClaw Skill — Demand Signal Miner v1.0.0 (rev-17-demand-signal-miner)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-17-demand-signal-miner-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("target_niches") or len(spec["inputs"]["target_niches"]) < 10:
        errors.append("Missing required input: target_niches")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    target_niches = spec["inputs"].get("target_niches", "")
    platforms = spec["inputs"].get("platforms", "twitter,instagram,tiktok,reddit,linkedin")
    context = f"""Task: Demand Signal Miner
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a demand extraction specialist. Mine social platforms for real buying signals, pain points, and unmet needs. Distinguish genuine demand from noise."""), ("human", f"""{context}{fb}""")]
    provider, model = get_routing_config()
...
def step_3_critic(state):
    ...
    if score < 7.0:
        try:
            from langchain_anthropic import ChatAnthropic
            resp = ChatAnthropic(model="claude-haiku-4-5-20251001", max_tokens=300).invoke([("system","Score 1-10 as JSON: {\"score\":N,\"feedback\":\"...\"}"),("human",f"Task: Demand Signal Miner\n\nOutput:\n{output[:2000]}")])
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-17-demand-signal-miner_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-17-demand-signal-miner_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-17-demand-signal-miner", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-17-demand-signal-miner"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-17-demand-signal-miner_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Demand Signal Miner v1.0.0\nThread: {wf_id}")
