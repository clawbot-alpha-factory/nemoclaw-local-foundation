#!/usr/bin/env python3
"""
NemoClaw Skill — Funnel Conversion Analyzer v1.0.0 (rev-05-funnel-conversion-analyzer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-05-funnel-conversion-analyzer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("funnel_data") or len(spec["inputs"]["funnel_data"]) < 20:
        errors.append("Missing required input: funnel_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    funnel_data = spec["inputs"].get("funnel_data", "")
    funnel_type = spec["inputs"].get("funnel_type", "sales")
    context = f"""Task: Funnel Conversion Analyzer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a funnel optimization expert. Identify conversion bottlenecks and produce specific, actionable fixes."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-05-funnel-conversion-analyzer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-05-funnel-conversion-analyzer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-05-funnel-conversion-analyzer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-05-funnel-conversion-analyzer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-05-funnel-conversion-analyzer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Funnel Conversion Analyzer v1.0.0\nThread: {wf_id}")
