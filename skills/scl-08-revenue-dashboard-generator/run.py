#!/usr/bin/env python3
"""
NemoClaw Skill — Revenue Dashboard Generator v1.0.0 (scl-08-revenue-dashboard-generator)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-scl-08-revenue-dashboard-generator-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("revenue_data") or len(spec["inputs"]["revenue_data"]) < 20:
        errors.append("Missing required input: revenue_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    revenue_data = spec["inputs"].get("revenue_data", "")
    report_type = spec["inputs"].get("report_type", "weekly")
    context = f"""Task: Revenue Dashboard Generator
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a revenue reporting specialist. Create clear, actionable dashboards that executives can use to make decisions in 5 minutes."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"scl-08-revenue-dashboard-generator_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"scl-08-revenue-dashboard-generator_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "scl-08-revenue-dashboard-generator", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="scl-08-revenue-dashboard-generator"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"scl-08-revenue-dashboard-generator_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Revenue Dashboard Generator v1.0.0\nThread: {wf_id}")
