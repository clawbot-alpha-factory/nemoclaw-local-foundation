#!/usr/bin/env python3
"""
NemoClaw Skill — TikTok Content Engine v1.0.0 (cnt-03-tiktok-content-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-03-tiktok-content-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("niche") or len(spec["inputs"]["niche"]) < 5:
        errors.append("Missing required input: niche")
    if errors:
...
def step_1_parse(state):
    ...
    batch_size = spec["inputs"].get("batch_size", "10")
    context = f"""Task: TikTok Content Engine
- niche: {niche}
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a TikTok content machine. Produce ready-to-film scripts in batch that follow trending formats and maximize viral potential."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-03-tiktok-content-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-03-tiktok-content-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-03-tiktok-content-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-03-tiktok-content-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-03-tiktok-content-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: TikTok Content Engine v1.0.0\nThread: {wf_id}")
