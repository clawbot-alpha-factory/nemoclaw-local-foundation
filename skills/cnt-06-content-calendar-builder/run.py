#!/usr/bin/env python3
"""
NemoClaw Skill — Content Calendar Builder v1.0.0 (cnt-06-content-calendar-builder)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-06-content-calendar-builder-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("business_context") or len(spec["inputs"]["business_context"]) < 20:
        errors.append("Missing required input: business_context")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    business_context = spec["inputs"].get("business_context", "")
    channels = spec["inputs"].get("channels", "linkedin,instagram,tiktok,youtube,email")
    context = f"""Task: Content Calendar Builder
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a content strategist. Build a comprehensive 30-day calendar that drives consistent engagement and conversions across all channels."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-06-content-calendar-builder_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-06-content-calendar-builder_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-06-content-calendar-builder", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-06-content-calendar-builder"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-06-content-calendar-builder_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Content Calendar Builder v1.0.0\nThread: {wf_id}")
