#!/usr/bin/env python3
"""
NemoClaw Skill — Revenue Attribution Analyzer v1.0.0 (rev-03-revenue-attribution-analyzer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-03-revenue-attribution-analyzer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def step_1_parse(state):
    ...
    period = spec["inputs"].get("period", "last_30_days")
    context = f"""Task: Revenue Attribution Analyzer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a revenue attribution analyst. Trace every conversion back to its origin with multi-touch attribution modeling."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-03-revenue-attribution-analyzer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-03-revenue-attribution-analyzer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-03-revenue-attribution-analyzer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-03-revenue-attribution-analyzer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-03-revenue-attribution-analyzer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Revenue Attribution Analyzer v1.0.0\nThread: {wf_id}")
