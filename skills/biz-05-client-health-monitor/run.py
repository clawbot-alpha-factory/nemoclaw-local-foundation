#!/usr/bin/env python3
"""
NemoClaw Skill — Client Health Monitor v1.0.0 (biz-05-client-health-monitor)
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("client_data") or len(spec["inputs"]["client_data"]) < 20:
        errors.append("Missing required input: client_data")
    if errors:
...
def step_1_parse(state):
    ...
    threshold = spec["inputs"].get("threshold", "70")
    context = f"""Task: Client Health Monitor
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a client success analyst. Monitor health indicators and flag at-risk clients before they churn."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"biz-05-client-health-monitor_{wf_id}_{ts}.md"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="biz-05-client-health-monitor"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"biz-05-client-health-monitor_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Client Health Monitor v1.0.0\nThread: {wf_id}")
