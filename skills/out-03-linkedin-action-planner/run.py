#!/usr/bin/env python3
"""
NemoClaw Skill — LinkedIn Action Planner v1.0.0 (out-03-linkedin-action-planner)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-03-linkedin-action-planner-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("prospect_linkedin") or len(spec["inputs"]["prospect_linkedin"]) < 10:
        errors.append("Missing required input: prospect_linkedin")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    prospect_linkedin = spec["inputs"].get("prospect_linkedin", "")
    approach = spec["inputs"].get("approach", "warm_connection")
    context = f"""Task: LinkedIn Action Planner
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a LinkedIn outreach specialist. Create personalized connection requests and messages that start real conversations."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-03-linkedin-action-planner_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-03-linkedin-action-planner_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-03-linkedin-action-planner", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-03-linkedin-action-planner"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-03-linkedin-action-planner_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: LinkedIn Action Planner v1.0.0\nThread: {wf_id}")
