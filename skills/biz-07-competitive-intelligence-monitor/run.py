#!/usr/bin/env python3
"""
NemoClaw Skill — Competitive Intelligence Monitor v1.0.0 (biz-07-competitive-intelligence-monitor)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-biz-07-competitive-intelligence-monitor-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("competitors") or len(spec["inputs"]["competitors"]) < 10:
        errors.append("Missing required input: competitors")
    if errors:
...
def step_1_parse(state):
    ...
    focus_areas = spec["inputs"].get("focus_areas", "pricing,features,messaging,hiring")
    context = f"""Task: Competitive Intelligence Monitor
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a competitive intelligence analyst. Monitor competitor movements and translate changes into actionable strategic recommendations."""), ("human", f"""{context}{fb}
...
def step_3_critic(state):
    ...
    if score < 7.0:
        try:
            from langchain_anthropic import ChatAnthropic
            resp = ChatAnthropic(model="claude-haiku-4-5-20251001", max_tokens=300).invoke([("system","Score 1-10 as JSON: {\"score\":N,\"feedback\":\"...\"}"),("human",f"Task: Competitive Intelligence Monitor\n\nOutput:\n{output[:2000]}")])
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"biz-07-competitive-intelligence-monitor_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"biz-07-competitive-intelligence-monitor_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "biz-07-competitive-intelligence-monitor", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="biz-07-competitive-intelligence-monitor"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"biz-07-competitive-intelligence-monitor_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Competitive Intelligence Monitor v1.0.0\nThread: {wf_id}")
