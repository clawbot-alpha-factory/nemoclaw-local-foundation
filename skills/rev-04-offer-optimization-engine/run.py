#!/usr/bin/env python3
"""
NemoClaw Skill — Offer Optimization Engine v1.0.0 (rev-04-offer-optimization-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-04-offer-optimization-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def step_1_parse(state):
    offer_performance_data = spec["inputs"].get("offer_performance_data", "")
    market_context = spec["inputs"].get("market_context", "B2B SaaS automation services")
    context = f"""Task: Offer Optimization Engine
- offer_performance_data: {offer_performance_data}
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a pricing and offer optimization specialist. Analyze conversion data to identify winning offer structures."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-04-offer-optimization-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-04-offer-optimization-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-04-offer-optimization-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-04-offer-optimization-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-04-offer-optimization-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Offer Optimization Engine v1.0.0\nThread: {wf_id}")
