#!/usr/bin/env python3
"""
NemoClaw Skill — Deal Progression Tracker v1.0.0 (rev-07-deal-progression-tracker)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-07-deal-progression-tracker-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("deals_data") or len(spec["inputs"]["deals_data"]) < 10:
        errors.append("Missing required input: deals_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    deals_data = spec["inputs"].get("deals_data", "")
    stale_threshold_days = spec["inputs"].get("stale_threshold_days", "3")
    context = f"""Task: Deal Progression Tracker
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a deal tracking specialist. Monitor every deal's health and recommend immediate actions for stale or at-risk opportunities."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-07-deal-progression-tracker_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-07-deal-progression-tracker_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-07-deal-progression-tracker", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
        "outputs": {"result": output, "result_summary": output[:300], "key_points": key_points, "skill_type": "tracker",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-07-deal-progression-tracker"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-07-deal-progression-tracker_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Deal Progression Tracker v1.0.0\nThread: {wf_id}")
