#!/usr/bin/env python3
"""
NemoClaw Skill — Viral Hook Generator v1.0.0 (cnt-01-viral-hook-generator)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-01-viral-hook-generator-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def step_1_parse(state):
    ...
    platform = spec["inputs"].get("platform", "instagram_reels")
    context = f"""Task: Viral Hook Generator
- topic: {topic}
- platform: {platform}"""
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a viral content strategist. Generate scroll-stopping hooks using proven psychological frameworks that drive engagement."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-01-viral-hook-generator_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-01-viral-hook-generator_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-01-viral-hook-generator", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-01-viral-hook-generator"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-01-viral-hook-generator_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Viral Hook Generator v1.0.0\nThread: {wf_id}")
