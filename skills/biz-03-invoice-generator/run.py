#!/usr/bin/env python3
"""
NemoClaw Skill — Invoice Generator v1.0.0 (biz-03-invoice-generator)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-biz-03-invoice-generator-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("contract_details") or len(spec["inputs"]["contract_details"]) < 20:
        errors.append("Missing required input: contract_details")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    contract_details = spec["inputs"].get("contract_details", "")
    payment_method = spec["inputs"].get("payment_method", "lemon_squeezy")
    context = f"""Task: Invoice Generator
...
def step_2_generate(state):
    ...
    messages = [("system", """You are an invoicing specialist. Generate clear, professional invoices with payment links and proper business details."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"biz-03-invoice-generator_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"biz-03-invoice-generator_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "biz-03-invoice-generator", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="biz-03-invoice-generator"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"biz-03-invoice-generator_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Invoice Generator v1.0.0\nThread: {wf_id}")
