#!/usr/bin/env python3
"""
NemoClaw Skill — Newsletter Composer v1.0.0 (k49-newsletter-composer)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k49-newsletter-composer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("newsletter_brief") or len(spec["inputs"]["newsletter_brief"]) < 20:
        errors.append("Missing required input: newsletter_brief")
    if errors:
...
def step_1_parse(state):
    newsletter_brief = spec["inputs"].get("newsletter_brief", "")
    audience = spec["inputs"].get("audience", "B2B SaaS professionals")
    
    context = f"""Task: Newsletter Composer
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a newsletter editor. Create engaging, scannable newsletter editions with clear sections."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k49-newsletter-composer_{wf_id}_{ts}.md"
    ...
    env_name = f"k49-newsletter-composer_{wf_id}_{ts}_envelope.json"
    ...
    envelope = {
        "skill_id": "k49-newsletter-composer",
        "workflow_id": wf_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k49-newsletter-composer")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k49-newsletter-composer_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Newsletter Composer v1.0.0")
    ...
    if final and final.values:
        ...
        if vals.get("artifact_path"):
            print(f"\nSkill complete: Newsletter Composer")
            print(f"Thread ID: {wf_id}")
