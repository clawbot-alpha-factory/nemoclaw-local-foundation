#!/usr/bin/env python3
"""
NemoClaw Skill — Comment Conversion Engine v1.0.0 (out-08-comment-conversion-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-08-comment-conversion-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("monitored_accounts") or len(spec["inputs"]["monitored_accounts"]) < 10:
        errors.append("Missing required input: monitored_accounts")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    monitored_accounts = spec["inputs"].get("monitored_accounts", "")
    response_tone = spec["inputs"].get("response_tone", "helpful_expert")
    context = f"""Task: Comment Conversion Engine
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a social engagement specialist. Turn public comments into private conversations and pipeline opportunities. Always lead with value, never spam."""), ("human", f"""{context}{fb}""")]
...
def step_3_critic(state):
    ...
    # Outbound safety DISABLED — agents have full autonomy (2026-04-02)
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-08-comment-conversion-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-08-comment-conversion-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-08-comment-conversion-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-08-comment-conversion-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-08-comment-conversion-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Comment Conversion Engine v1.0.0\nThread: {wf_id}")
