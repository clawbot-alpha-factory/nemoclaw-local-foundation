#!/usr/bin/env python3
"""
NemoClaw Skill — Live Experiment Runner v1.0.0 (rev-13-live-experiment-runner)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-13-live-experiment-runner-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("experiment_spec") or len(spec["inputs"]["experiment_spec"]) < 20:
        errors.append("Missing required input: experiment_spec")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    experiment_spec = spec["inputs"].get("experiment_spec", "")
    confidence_threshold = spec["inputs"].get("confidence_threshold", "0.90")
    context = f"""Task: Live Experiment Runner
...
def step_2_generate(state):
    ...
    messages = [("system", """You are an experimentation specialist. Design, execute, and analyze A/B tests with statistical rigor. Declare winners only with sufficient confidence."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-13-live-experiment-runner_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-13-live-experiment-runner_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-13-live-experiment-runner", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-13-live-experiment-runner"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-13-live-experiment-runner_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Live Experiment Runner v1.0.0\nThread: {wf_id}")
