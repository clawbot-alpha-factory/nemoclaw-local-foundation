#!/usr/bin/env python3
"""
NemoClaw Skill — Follow-Up Enforcer v1.0.0 (rev-11-follow-up-enforcer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-11-follow-up-enforcer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("deal_context") or len(spec["inputs"]["deal_context"]) < 20:
        errors.append("Missing required input: deal_context")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    deal_context = spec["inputs"].get("deal_context", "")
    days_since_contact = spec["inputs"].get("days_since_contact", "3")
    context = f"""Task: Follow-Up Enforcer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a relentless follow-up specialist. No deal goes cold on your watch. Generate contextual, escalating follow-ups that move deals forward."""), ("human", f"""{context}{fb}""")]
    provider, model = get_routing_config()
...
def step_3_critic(state):
    ...
    if score < 7.0:
        try:
            from langchain_anthropic import ChatAnthropic
            resp = ChatAnthropic(model="claude-haiku-4-5-20251001", max_tokens=300).invoke([("system","Score 1-10 as JSON: {\"score\":N,\"feedback\":\"...\"}"),("human",f"Task: Follow-Up Enforcer\n\nOutput:\n{output[:2000]}")])
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-11-follow-up-enforcer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-11-follow-up-enforcer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-11-follow-up-enforcer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-11-follow-up-enforcer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-11-follow-up-enforcer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Follow-Up Enforcer v1.0.0\nThread: {wf_id}")
