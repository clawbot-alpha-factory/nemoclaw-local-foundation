#!/usr/bin/env python3
"""
NemoClaw Skill — Lead Qualification Engine v1.0.0 (rev-02-lead-qualification-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-02-lead-qualification-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def step_1_parse(state):
    ...
    icp_criteria = spec["inputs"].get("icp_criteria", "B2B SaaS, 10-500 employees, MENA or global, needs automation")
    context = f"""Task: Lead Qualification Engine
- lead_data: {lead_data}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-02-lead-qualification-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-02-lead-qualification-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-02-lead-qualification-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-02-lead-qualification-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-02-lead-qualification-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Lead Qualification Engine v1.0.0\nThread: {wf_id}")
