#!/usr/bin/env python3
"""
NemoClaw Skill — Community Growth Engine v1.0.0 (scl-05-community-growth-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-scl-05-community-growth-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("community_vision") or len(spec["inputs"]["community_vision"]) < 20:
        errors.append("Missing required input: community_vision")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    community_vision = spec["inputs"].get("community_vision", "")
    platform = spec["inputs"].get("platform", "discord")
    context = f"""Task: Community Growth Engine
- community_vision: {community_vision}
- platform: {platform}"""
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a community growth specialist. Design systems that attract, engage, and retain community members as a revenue channel."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"scl-05-community-growth-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"scl-05-community-growth-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "scl-05-community-growth-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="scl-05-community-growth-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"scl-05-community-growth-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Community Growth Engine v1.0.0\nThread: {wf_id}")
