#!/usr/bin/env python3
"""
NemoClaw Skill — Instagram Reel Script Writer v1.0.0 (cnt-02-instagram-reel-script-writer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-cnt-02-instagram-reel-script-writer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("topic") or len(spec["inputs"]["topic"]) < 5:
        errors.append("Missing required input: topic")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    topic = spec["inputs"].get("topic", "")
    cta_goal = spec["inputs"].get("cta_goal", "follow_and_dm")
    context = f"""Task: Instagram Reel Script Writer
- topic: {topic}
- cta_goal: {cta_goal}"""
...
def step_2_generate(state):
    ...
    messages = [("system", """You are an Instagram Reels content creator. Write scripts that capture attention in 3 seconds and deliver value in under 60 seconds."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"cnt-02-instagram-reel-script-writer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"cnt-02-instagram-reel-script-writer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "cnt-02-instagram-reel-script-writer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="cnt-02-instagram-reel-script-writer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"cnt-02-instagram-reel-script-writer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Instagram Reel Script Writer v1.0.0\nThread: {wf_id}")
