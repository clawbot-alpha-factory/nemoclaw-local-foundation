#!/usr/bin/env python3
"""
NemoClaw Skill — Blog Post Writer v1.0.0 (k48-blog-post-writer)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k48-blog-post-writer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("topic") or len(spec["inputs"]["topic"]) < 10:
...
def step_1_parse(state):
    ...
    context = f"""Task: Blog Post Writer
- topic: {topic}
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a content marketing writer specializing in SEO-optimized B2B blog content."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k48-blog-post-writer_{wf_id}_{ts}.md"
    ...
    env_name = f"k48-blog-post-writer_{wf_id}_{ts}_envelope.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k48-blog-post-writer")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k48-blog-post-writer_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Blog Post Writer v1.0.0")
    ...
    if final and final.values:
        ...
        if vals.get("artifact_path"):
            print(f"\nSkill complete: Blog Post Writer")
            print(f"Thread ID: {wf_id}")
