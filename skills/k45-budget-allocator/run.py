#!/usr/bin/env python3
"""
NemoClaw Skill — Budget Allocator v1.0.0 (k45-budget-allocator)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k45-budget-allocator-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("channel_performance") or len(spec["inputs"]["channel_performance"]) < 20:
        errors.append("Missing required input: channel_performance")
    if not spec["inputs"].get("total_budget") or len(spec["inputs"]["total_budget"]) < 3:
        errors.append("Missing required input: total_budget")
    if errors:
...
def step_1_parse(state):
    channel_performance = spec["inputs"].get("channel_performance", "")
    total_budget = spec["inputs"].get("total_budget", "")
    
    context = f"""Task: Budget Allocator
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a marketing budget optimization specialist. Produce data-driven channel allocation recommendations."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k45-budget-allocator_{wf_id}_{ts}.md"
    ...
    env_name = f"k45-budget-allocator_{wf_id}_{ts}_envelope.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k45-budget-allocator")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k45-budget-allocator_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Budget Allocator v1.0.0")
    ...
    if final and final.values:
        ...
        if vals.get("artifact_path"):
            print(f"\nSkill complete: Budget Allocator")
            print(f"Thread ID: {wf_id}")
