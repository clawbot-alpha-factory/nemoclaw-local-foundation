#!/usr/bin/env python3
"""
NemoClaw Skill — Webinar Funnel Builder v1.0.0 (scl-03-webinar-funnel-builder)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-scl-03-webinar-funnel-builder-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("target_audience") or len(spec["inputs"]["target_audience"]) < 10:
        errors.append("Missing required input: target_audience")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    webinar_topic = spec["inputs"].get("webinar_topic", "")
    target_audience = spec["inputs"].get("target_audience", "")
    context = f"""Task: Webinar Funnel Builder
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a webinar marketing expert. Build complete webinar funnels that convert registrants into paying customers."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"scl-03-webinar-funnel-builder_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"scl-03-webinar-funnel-builder_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "scl-03-webinar-funnel-builder", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="scl-03-webinar-funnel-builder"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"scl-03-webinar-funnel-builder_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Webinar Funnel Builder v1.0.0\nThread: {wf_id}")
