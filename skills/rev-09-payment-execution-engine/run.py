#!/usr/bin/env python3
"""
NemoClaw Skill — Payment Execution Engine v1.0.0 (rev-09-payment-execution-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-rev-09-payment-execution-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("invoice_data") or len(spec["inputs"]["invoice_data"]) < 20:
        errors.append("Missing required input: invoice_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    invoice_data = spec["inputs"].get("invoice_data", "")
    payment_provider = spec["inputs"].get("payment_provider", "lemon_squeezy")
    context = f"""Task: Payment Execution Engine
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a payment operations specialist. Generate payment links, track status, and ensure seamless payment-to-onboarding transitions."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"rev-09-payment-execution-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"rev-09-payment-execution-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "rev-09-payment-execution-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="rev-09-payment-execution-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"rev-09-payment-execution-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Payment Execution Engine v1.0.0\nThread: {wf_id}")
