#!/usr/bin/env python3
"""
NemoClaw Skill — Campaign Performance Optimizer v1.0.0 (out-06-campaign-performance-optimizer)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-06-campaign-performance-optimizer-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("campaign_data") or len(spec["inputs"]["campaign_data"]) < 20:
        errors.append("Missing required input: campaign_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    campaign_data = spec["inputs"].get("campaign_data", "")
    min_sample_size = spec["inputs"].get("min_sample_size", "50")
    context = f"""Task: Campaign Performance Optimizer
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a campaign optimization specialist. Analyze performance data and make decisive recommendations: kill, scale, or iterate."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-06-campaign-performance-optimizer_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-06-campaign-performance-optimizer_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-06-campaign-performance-optimizer", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-06-campaign-performance-optimizer"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-06-campaign-performance-optimizer_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Campaign Performance Optimizer v1.0.0\nThread: {wf_id}")
