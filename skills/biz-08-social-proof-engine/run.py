#!/usr/bin/env python3
"""
NemoClaw Skill — Social Proof Engine v1.0.0 (biz-08-social-proof-engine)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-biz-08-social-proof-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("client_results") or len(spec["inputs"]["client_results"]) < 20:
        errors.append("Missing required input: client_results")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    client_results = spec["inputs"].get("client_results", "")
    output_formats = spec["inputs"].get("output_formats", "case_study,testimonial,before_after,quote_card")
    context = f"""Task: Social Proof Engine
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a social proof specialist. Transform raw client results into compelling proof assets that accelerate trust and conversion at every touchpoint."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"biz-08-social-proof-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"biz-08-social-proof-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "biz-08-social-proof-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="biz-08-social-proof-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"biz-08-social-proof-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Social Proof Engine v1.0.0\nThread: {wf_id}")
