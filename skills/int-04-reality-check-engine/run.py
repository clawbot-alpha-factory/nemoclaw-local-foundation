#!/usr/bin/env python3
"""
NemoClaw Skill — Reality Check Engine v1.0.0 (int-04-reality-check-engine)
...
def call_llm(messages, max_tokens=4000):
    ...
    return call_llm_or_chain(messages, task_class="premium", task_domain="strategic_reasoning", max_tokens=max_tokens)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-int-04-reality-check-engine-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("opportunity_data") or len(spec["inputs"]["opportunity_data"]) < 20:
        errors.append("Missing required input: opportunity_data")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    opportunity_data = spec["inputs"].get("opportunity_data", "")
    historical_results = spec["inputs"].get("historical_results", "")
    context = f"""Task: Reality Check Engine
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a critical analyst and devil's advocate. Challenge every assumption with data. Your job is to prevent the system from chasing false opportunities."""), ("human", f"""{context}{fb}
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"int-04-reality-check-engine_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"int-04-reality-check-engine_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "int-04-reality-check-engine", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="int-04-reality-check-engine"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"int-04-reality-check-engine_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Reality Check Engine v1.0.0\nThread: {wf_id}")
