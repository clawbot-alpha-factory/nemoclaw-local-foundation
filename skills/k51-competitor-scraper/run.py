#!/usr/bin/env python3
"""
NemoClaw Skill — Competitor Scraper v1.0.0 (k51-competitor-scraper)
...
def load_spec(args):
    spec = {
        "inputs": {},
        "context": {
            "workflow_id": args.thread_id or f"skill-k51-competitor-scraper-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}",
...
def validate_inputs(spec):
    errors = []
    if not spec["inputs"].get("competitors") or len(spec["inputs"]["competitors"]) < 5:
...
def step_2_generate(state):
    ...
    messages = [
        ("system", """You are a competitive intelligence analyst. Produce detailed competitor comparison reports."""),
...
def step_5_artifact(state):
    ...
    md_name = f"k51-competitor-scraper_{wf_id}_{ts}.md"
    ...
    env_name = f"k51-competitor-scraper_{wf_id}_{ts}_envelope.json"
    ...
    envelope = {
        "skill_id": "k51-competitor-scraper",
        "workflow_id": wf_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="k51-competitor-scraper")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"k51-competitor-scraper_*_envelope.json"))
        if existing:
    ...
    print(f"\nStarting skill: Competitor Scraper v1.0.0")
    ...
    if final and final.values:
        ...
        if vals.get("artifact_path"):
            print(f"\nSkill complete: Competitor Scraper")
            print(f"Thread ID: {wf_id}")
