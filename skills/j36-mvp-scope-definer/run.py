...
def call_openai(messages, model=None, max_tokens=6000):
    ...
    return call_llm_or_chain(messages, task_class="complex_reasoning", task_domain="strategic_reasoning", max_tokens=max_tokens)
...
def step_3_critic(inputs, context):
    ...
    generated_scope = context.get("improved_scope", context.get("generated_scope", ""))
    if not generated_scope:
        return None, "Missing scope document (no generated_scope or improved_scope)"
...
{{
  ...
  "improvement_suggestions": ["...", "..."]
    ...
    try:
        ...
        scores = {
            "prioritization_quality": 3,
            "actionability": 3,
            "constraint_grounding": 3,
            "weaknesses": ["Could not parse critic LLM response — defaulting to low scores"],
