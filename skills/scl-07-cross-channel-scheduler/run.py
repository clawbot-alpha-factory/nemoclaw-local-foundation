#!/usr/bin/env python3
"""
NemoClaw Skill — Cross-Channel Scheduler v1.0.0 (scl-07-cross-channel-scheduler)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-scl-07-cross-channel-scheduler-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def validate_inputs(spec):
    ...
    if not spec["inputs"].get("content_queue") or len(spec["inputs"]["content_queue"]) < 20:
        errors.append("Missing required input: content_queue")
    if errors:
        for e in errors: print(f"  ERROR: {e}")
        sys.exit(1)

def step_1_parse(state):
    content_queue = spec["inputs"].get("content_queue", "")
    timezone = spec["inputs"].get("timezone", "Asia/Amman")
    context = f"""Task: Cross-Channel Scheduler
...
def step_2_generate(state):
    ...
    messages = [("system", """You are a social media scheduling specialist. Optimize posting times per platform and coordinate cross-channel distribution."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"scl-07-cross-channel-scheduler_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"scl-07-cross-channel-scheduler_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "scl-07-cross-channel-scheduler", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="scl-07-cross-channel-scheduler"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"scl-07-cross-channel-scheduler_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Cross-Channel Scheduler v1.0.0\nThread: {wf_id}")
