#!/usr/bin/env python3
"""
NemoClaw Skill — Email Executor v1.0.0 (out-02-email-executor)
...
def load_spec(args):
    spec = {"inputs": {}, "context": {"workflow_id": args.thread_id or f"skill-out-02-email-executor-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}", "budget_state": {"remaining": 20.0}, "step_history": []}}
    if args.input:
...
def step_1_parse(state):
    email_spec = spec["inputs"].get("email_spec", "")
    sequence_context = spec["inputs"].get("sequence_context", "")
    context = f"""Task: Email Executor
...
def step_2_generate(state):
    ...
    messages = [("system", """You are an email delivery specialist. Compose and send personalized emails, track engagement, and trigger follow-up actions based on recipient behavior."""), ("human", f"""{context}{fb}""")]
...
def step_5_artifact(state):
    ...
    md_path = OUTPUTS_DIR / f"out-02-email-executor_{wf_id}_{ts}.md"
    md_path.write_text(output)
    key_points = [l.lstrip("- *0123456789.").strip() for l in output.split("\n") if l.strip().startswith(("- ","1.","* "))][:10]
    env_path = OUTPUTS_DIR / f"out-02-email-executor_{wf_id}_{ts}_envelope.json"
    envelope = {"skill_id": "out-02-email-executor", "workflow_id": wf_id, "timestamp": datetime.now(timezone.utc).isoformat(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skill", default="out-02-email-executor"); parser.add_argument("--input", nargs=2, action="append")
    ...
    if not args.force:
        existing = list(OUTPUTS_DIR.glob(f"out-02-email-executor_*_envelope.json"))
        if existing: print(f"Skip: Output exists ({sorted(existing)[-1].name}). Use --force."); sys.exit(0)
    print(f"\nStarting: Email Executor v1.0.0\nThread: {wf_id}")
